...
plot_vish_image <- function(gene, thres) {
  ggplot(e14.5_geometry, aes(x = x, y = -z)) + geom_image(x = 25.5, y = -27, image = e14.5_image, size = 1) + geom_point(aes(color =  computeVISH(e14.5_spatial, gene, threshold = thres)), size = 1.5) + scale_color_gradient(low = "lightgrey", high = "#009933") + theme_classic() + theme(legend.position = "none", axis.line = element_blank(), axis.title = element_blank(), axis.text = element_blank(), axis.ticks = element_blank()) + ggtitle(gene) + theme(plot.title = element_text(size = 15, face = "bold", hjust = 0.5, vjust = -2))
}
...
shinyServer(function(input, output, session) { 
 ...
 optCrt="{ option_create: function(data,escape) {return('<div class=\"create\"><strong>' + '</strong></div>');} }" 
  ...
  updateSelectizeInput(session, "vish", choices = names(sc1gene), server = TRUE, 
                       selected = "Sox9", options = list( 
                         maxOptions = 7, create = TRUE, persist = TRUE, render = I(optCrt)))
